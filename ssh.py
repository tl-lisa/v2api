...
def createAccount(env, accountPrefix, begNum, endNum, role):
    header = {'Connection': 'Keep-alive', 'X-Auth-Token': '', 'X-Auth-Nonce': ''}
    result = api.user_login(env, 'tl-lisa', '123456')
    header['X-Auth-Token'] = result['token']
    header['X-Auth-Nonce'] = result['nonce']            
    idlist = []
    for i in range(begNum, endNum):
        i += 1
        if len(str(i)) == 1:
            account = accountPrefix + '000' + str(i)
        elif len(str(i)) == 2:
            account = accountPrefix + '00' + str(i)
        elif len(str(i)) == 3:
            account = accountPrefix + '0' + str(i)
        else:
            account = accountPrefix + str(i)
    ...
    body = {'ids': idlist, 'role': role}
    api.apiFunction(env, header, url, 'patch', body)
    return
...
if __name__ == '__main__':
    createAccount('http://35.234.6.138', 'liveAcc', 10, 30, 4)
