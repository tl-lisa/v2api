...
Msg = {
    ...
    401:{'Status': 'Error', 'Message': 'AUTH_ERROR'},
    403:{'Status': 'Error', 'Message': 'PERMISSION_REQUIRED'},
    400:{'Status': 'Error', 'Message': 'PARAM_TYPE_ERROR'},
...
cover_list = [
    'https://d3eq1e23ftm9f0.cloudfront.net/namecard/photo/460d7772e0ec11e9beff42010a8c0fcc',
...
def teardown_module():
    ...
    api.changeRole(test_parameter['backend_nonce'], test_parameter['backend_token'], test_parameter['backend_nonce'], changelist, 4)#轉回直播主
...
def getTestData(testName): 
    ...
    if testName == 'addPhoto': 
        ...
        testData = [
            ...
            ('直播主上傳動態_影音檔但未設禮物類別', 'broadcaster_token', 'broadcaster_nonce', photo.createBody('video', '', '動態影片上傳', urlList[1], urlList[2], ''), 200),
            ...
            ('直播主上傳動態_影音檔，但預覧url空值', 'broadcaster_token', 'broadcaster_nonce', photo.createBody('video', '', '動態影片上傳', urlList[1], '', '108'), 406),
            ('直播主上傳動態_照片，但禮物類別不存在', 'broadcaster_token', 'broadcaster_nonce', photo.createBody('photo', urlList[0], '動態照片上傳', '', '', '333'), 404),
        ...
        testData = [
            ...
            ('取得該直播主動態列表_有資料', 'user_token', 'user_nonce', 0, 10, 1, 3, '', 200),
            ('原直播主轉成一般user，其動態無法再取得', 'user_token', 'user_nonce', 0, 10, 1, 3, 'changeRole', 400),
            ('user被列入黑名單，無法取得該直播主的動態', 'user_token', 'user_nonce', 0, 10, 1, 3, 'black', 400),
        ...
        testData = [
            ...
            ('user被列入黑名單，無法取得該直播主的動態', 'user_token', 'user_nonce', 1, 'black', False, 0, 10, '', 0, 400),
        ...
        testData = [
            ...
            ('送禮者雖被列入黑名單，仍可在送禮記錄裡查到', 'broadcaster_token', 'broadcaster_nonce', 3, 'black', 1, 200),
...
class TestPhotoOfMaster():
    ...
    def testUpdate(self, scenario, token, nonce, body, origenal, action, expected):
        actionDic = {
            'block': {'funName': api.blockUser, 'parameter': [test_parameter['prefix'], test_parameter[token], test_parameter[nonce], idlist[2]]},
...
class TestPhotoOperate():
    ...
    def testGetPhotolist(self, scenario, token, nonce, masterId, items, pages, totalCount, action, expected):
        actionDic = {
            'block': {'funName': api.blockUser, 'parameter': [test_parameter['prefix'], test_parameter['broadcaster_token'], test_parameter['broadcaster_nonce'], idlist[2]]},
            'changeRole': {'funName': api.changeRole, 'parameter': [test_parameter['prefix'], test_parameter['backend_token'], test_parameter['backend_nonce'], [idlist[0]], 5]}
        }
        if action != '':
            actionDic[action]['funName'](*actionDic[action]['parameter'])
        time.sleep(30)
        res = photo.getPhotoList(test_parameter[token], test_parameter[nonce], test_parameter['prefix'], idlist[0], items, pages)
        restext = json.loads(res.text)
        assert res.status_code == expected
        assert restext['Status'] == Msg[expected]['Status']
        assert restext['Message'] in Msg[expected]['Message']
            ...
            if totalCount == 0:
                ...
                for i in len(restext):
                    ...
                    if restext['data'][i]['type'] == 'photo':
                        ...
                        assert restext['data'][i]['videoPath'] == self.createPhotoList[abs(i-2)][3]
                        assert restext['data'][i]['previewPath'] == self.createPhotoList[abs(i-2)][4]
                        assert restext['data'][i]['giftCategoryId'] == int(self.createPhotoList[abs(i-2)][5])
