...
class TestNotificationSetting():
    ...
    def testGetsetting_case1(self):
        ...
        restext = json.loads(res.text)
        assert res.status_code == 200
        restext['data']['comment'] == True
        restext['data']['track'] == True
        restext['data']['live'] == True
        #print(time.strftime("%Y-%m-%d ", time.localtime(int(time.time()))))

    def testGetsetting_case2(self):
        #一般粉絲只有live是true
        header['Content-Type'] = ''
        header['X-Auth-Token'] = test_parameter['user_token']
        header['X-Auth-Nonce'] = test_parameter['user_nonce']
        ...
        restext = json.loads(res.text)
        assert res.status_code == 200
        restext['data']['comment'] == False
        restext['data']['track'] == False
        restext['data']['live'] == True

    def testGetsetting_case3(self):
        #直播主轉成一般粉絲只有live是true
        header['Content-Type'] = 'application/json'
        header['X-Auth-Token'] = test_parameter['backend_token']
        header['X-Auth-Nonce'] = test_parameter['backend_nonce']   
        mid = api.search_user(test_parameter['prefix'], test_parameter['broadcaster_acc'], header)     
        changelist = [] 
        changelist.append(mid) 
        res = api.change_roles(test_parameter['prefix'], header, changelist, '5') #轉成一般用戶
        header['Content-Type'] = ''
        header['X-Auth-Token'] = test_parameter['broadcaster_token']
        header['X-Auth-Nonce'] = test_parameter['broadcaster_nonce']
        apiNmae = '/api/v2/identity/notifySetting'
        ...
        header['X-Auth-Token'] = test_parameter['backend_token']
        header['X-Auth-Nonce'] = test_parameter['backend_nonce']   
        res = api.change_roles(test_parameter['prefix'], header, changelist, '4') #轉成直播主
    
    def testEditsetting_case1(self):
        #一般user修改設定
        header['Content-Type'] = 'application/json'
        header['X-Auth-Token'] = test_parameter['user_token']
        header['X-Auth-Nonce'] = test_parameter['user_nonce']
        apiNmae = '/api/v2/identity/notifySetting'
        body = {'comment': True, 'track': True,  'live': True}
    ...
    def testEditsetting_case2(self):
        #直播主修改設定
        header['Content-Type'] = 'application/json'
        header['X-Auth-Token'] = test_parameter['broadcaster_token']
        header['X-Auth-Nonce'] = test_parameter['broadcaster_nonce']
        apiNmae = '/api/v2/identity/notifySetting'
        body = {'comment': True, 'track': True,  'live': False}
        res = api.apiFunction(test_parameter['prefix'], header, apiNmae, 'post', body)
        assert res.status_code == 200
        header['Content-Type'] = ''    
        res = api.apiFunction(test_parameter['prefix'], header, apiNmae, 'get', None)    
        ...
        restext['data']['comment'] == True
        restext['data']['track'] == True
        restext['data']['live'] == False    

    def testEditsetting_case3(self):
        #直播主轉成一般user
        header['Content-Type'] = 'application/json'
        header['X-Auth-Token'] = test_parameter['backend_token']
        header['X-Auth-Nonce'] = test_parameter['backend_nonce']   
    ...
    def testEditsetting_case4(self):
        #token/nonce不存在
        header['Content-Type'] = 'application/json'
    ...
    def testEditsetting_case5(self):
        #json error
        header['Content-Type'] = 'application/json'
        header['X-Auth-Token'] = test_parameter['broadcaster_token']
        header['X-Auth-Nonce'] = test_parameter['broadcaster_nonce']
    ...
    def testEditsetting_case6(self):
        #key value is none
        header['Content-Type'] = 'application/json'
        header['X-Auth-Token'] = test_parameter['broadcaster_token']
        header['X-Auth-Nonce'] = test_parameter['broadcaster_nonce']
        ...
        res = api.apiFunction(test_parameter['prefix'], header, apiNmae, 'get', None)
        restext = json.loads(res.text)
        track = restext['data']['track']
        body = {'comment': True, 'track': None,  'live': False}        
        res = api.apiFunction(test_parameter['prefix'], header, apiNmae, 'post', body)
        restext = json.loads(res.text)
        assert int(res.status_code / 100) == 2
        restext['data']['comment'] == True
        restext['data']['track'] == track
        restext['data']['live'] == False    

    def testEditsetting_case7(self):
        #key value is empty
        header['Content-Type'] = 'application/json'
        header['X-Auth-Token'] = test_parameter['broadcaster_token']
        header['X-Auth-Nonce'] = test_parameter['broadcaster_nonce']
        ...
        restext = json.loads(res.text)
        track = restext['data']['track']
        body = {'comment': True, 'track': '',  'live': False}
        res = api.apiFunction(test_parameter['prefix'], header, apiNmae, 'post', body)
        restext = json.loads(res.text)
        assert int(res.status_code / 100) == 2
        restext['data']['comment'] == True
